...
// .wrangler/tmp/bundle-Y2GYOF/checked-fetch.js
...
__name(isPointInside, "isPointInside");
function hashVertices(vertices) {
  return createHash("sha256").update(JSON.stringify(vertices)).digest("hex");
}
__name(hashVertices, "hashVertices");
var LIVE_ZONE_MAX = 500;
var FREEZE_BATCH = 250;
function checkFreeze(liveZone, existingBase) {
...
__name(checkFreeze, "checkFreeze");

// shared/heapTypes.ts
var DEFAULT_HEAP_PARAMS = {
  name: "Unnamed Heap",
  difficulty: 1,
  spawnRateMult: 1,
  coinMult: 1,
...
__name(resolveParams, "resolveParams");
function heapRoutes(db) {
  const app = new Hono2();
  app.post("/", async (c) => {
    let body;
    try {
    ...
    if (!Array.isArray(vertices) || vertices.length < 3 || !vertices.every((v) => typeof v?.x === "number" && typeof v?.y === "number")) {
  ...
  app.get("/:id", async (c) => {
    ...
    if (clientVersion === row.version) {
      return c.json({ changed: false, version: row.version });
    }
    const [liveZone, enemyParams] = await Promise.all([
      Promise.resolve(JSON.parse(row.live_zone)),
  ...
  app.post("/:id/place", async (c) => {
    ...
    const row = await db.getHeap(id);
    if (!row) return c.json({ error: "Heap not found" }, 404);
    const liveZone = JSON.parse(row.live_zone);
    ...
    if (isPointInside({ x, y }, fullPolygon)) {
      return c.json({ accepted: false, version: row.version });
    }
    const newVertex = { x, y };
    ...
    return c.json({ accepted: true, version: newVersion });
  });
...
__name(heapRoutes, "heapRoutes");

// server/src/routes/scores.ts
var DEFAULT_LIMIT = 5;
var MAX_LIMIT = 50;
async function buildContext(db, heapId, playerId, limit) {
  ...
  const rank = await db.getRank(heapId, playerRow.score);
  const player = {
    rank,
...
function scoreRoutes(db) {
  ...
  app.post("/", async (c) => {
    ...
    return c.json({ submitted, context });
  });
...
__name(scoreRoutes, "scoreRoutes");

// server/src/app.ts
function createApp(heapDb, scoreDb) {
  const app = new Hono2();
  app.use("*", cors());
...
var D1HeapDB = class {
  ...
  async createHeap(heapId, baseId, vertices, vertexHash, now, params = DEFAULT_HEAP_PARAMS) {
    await this.d1.batch([
      this.d1.prepare(
  ...
  async updateHeap(id, baseId, version, liveZone, freezeY) {
    await this.d1.prepare("UPDATE heap SET base_id = ?1, version = ?2, live_zone = ?3, freeze_y = ?4 WHERE id = ?5").bind(baseId, version, JSON.stringify(liveZone), freezeY, id).run();
  }
  async updateHeapParams(id, params) {
    await this.d1.prepare(
      `UPDATE heap SET name = ?1, difficulty = ?2, spawn_rate_mult = ?3, coin_mult = ?4, score_mult = ?5, world_height = ?6
         WHERE id = ?7`
    ).bind(params.name, params.difficulty, params.spawnRateMult, params.coinMult, params.scoreMult, params.worldHeight, id).run();
  }
  async deleteHeap(id) {
    await this.d1.batch([
  ...
  async getBaseVerticesById(baseId) {
    const row = await this.d1.prepare("SELECT vertices FROM heap_base WHERE id = ?1").bind(baseId).first();
    return row ? JSON.parse(row.vertices) : null;
  }
  async createBase(id, heapId, vertices, vertexHash, now) {
    await this.d1.prepare("INSERT INTO heap_base (id, heap_id, vertices, vertex_hash, created_at) VALUES (?1, ?2, ?3, ?4, ?5)").bind(id, heapId, JSON.stringify(vertices), vertexHash, now).run();
  }
  async getEnemyParams(heapId) {
    const row = await this.d1.prepare("SELECT enemy_params FROM heap_parameters WHERE heap_id = ?1").bind(heapId).first();
    if (row) return JSON.parse(row.enemy_params);
    const sentinel = await this.d1.prepare("SELECT enemy_params FROM heap_parameters WHERE heap_id = '00000000-0000-0000-0000-000000000000'").first();
    return sentinel ? JSON.parse(sentinel.enemy_params) : {};
  }
  async upsertEnemyParams(heapId, params) {
    await this.d1.prepare(
    ...
    ).bind(heapId, JSON.stringify(params)).run();
  }
};
...
var D1ScoreDB = class {
  ...
  static {
    __name(this, "D1ScoreDB");
  }
  async getScore(heapId, playerId) {
    const row = await this.d1.prepare("SELECT * FROM score WHERE heap_id=?1 AND player_id=?2").bind(heapId, playerId).first();
    return row ?? null;
  }
  async upsertScore(heapId, playerId, name, score, now) {
    const existing = await this.getScore(heapId, playerId);
    if (existing && score <= existing.score) return false;
    if (existing) {
      await this.d1.prepare("UPDATE score SET name=?1, score=?2, updated_at=?3 WHERE heap_id=?4 AND player_id=?5").bind(name, score, now, heapId, playerId).run();
    } else {
      await this.d1.prepare("INSERT INTO score (heap_id, player_id, name, score, created_at, updated_at) VALUES (?1,?2,?3,?4,?5,?6)").bind(heapId, playerId, name, score, now, now).run();
    }
    return true;
  }
  async getTopScores(heapId, limit) {
    const result = await this.d1.prepare("SELECT * FROM score WHERE heap_id=?1 ORDER BY score DESC LIMIT ?2").bind(heapId, limit).all();
    return result.results;
  }
  async getRank(heapId, score) {
    const result = await this.d1.prepare("SELECT COUNT(*) as cnt FROM score WHERE heap_id=?1 AND score>?2").bind(heapId, score).first();
    return (result?.cnt ?? 0) + 1;
  }
  async countScores(heapId) {
    const result = await this.d1.prepare("SELECT COUNT(*) as cnt FROM score WHERE heap_id=?1").bind(heapId).first();
    return result?.cnt ?? 0;
  }
  async pruneScores(heapId) {
    await this.d1.prepare(`
        DELETE FROM score
        WHERE heap_id=?1
      ...
      `).bind(heapId, heapId).run();
  }
  async getScoresPaginated(heapId, offset, limit) {
    const result = await this.d1.prepare("SELECT * FROM score WHERE heap_id=?1 ORDER BY score DESC LIMIT ?2 OFFSET ?3").bind(heapId, limit, offset).all();
    return result.results;
  }
};
...
// .wrangler/tmp/bundle-Y2GYOF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Y2GYOF/middleware-loader.entry.ts
