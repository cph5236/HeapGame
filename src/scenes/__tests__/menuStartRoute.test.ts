...
describe('resolveMenuStart', () => {
  it('starts the tutorial while it is pending, whatever heap is loaded behind it', () => {
    expect(resolveMenuStart({ tutorialPending: true, isInfinite: false, hasCheckpoint: true }))
      .toEqual({ scene: 'TutorialScene' });
    expect(resolveMenuStart({ tutorialPending: true, isInfinite: true, hasCheckpoint: false }))
      .toEqual({ scene: 'TutorialScene' });
  });

  it('starts Infinite mode for an infinite heap', () => {
    expect(resolveMenuStart({ tutorialPending: false, isInfinite: true, hasCheckpoint: false }))
      .toEqual({ scene: 'InfiniteGameScene' });
  });

  it('starts a finite run, resuming from a checkpoint when one has spawns left', () => {
    expect(resolveMenuStart({ tutorialPending: false, isInfinite: false, hasCheckpoint: true }))
      .toEqual({ scene: 'GameScene', useCheckpoint: true });
    expect(resolveMenuStart({ tutorialPending: false, isInfinite: false, hasCheckpoint: false }))
